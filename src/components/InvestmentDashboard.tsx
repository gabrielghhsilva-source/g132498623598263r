...
import { InvestmentArea, Investment, InvestmentGoal } from "@/lib/types";
import { getAreaTotals, calculateGrowth, simulateUntilDate, getCurrentValue } from "@/lib/investmentCalc";
import { Plus, Trash2, ChevronDown, ChevronRight, Target, TrendingUp, DollarSign, Calendar, BarChart3 } from "lucide-react";
...
export function InvestmentDashboard({
  ...
  onAddContribution, onAddGoal, onDeleteGoal, onCreateTaskReminder,
}: Props) {
      ...
      {showAddArea ? (
        <div className="glass-card rounded-xl p-4 space-y-3 animate-fade-in">
          <input
            ...
            placeholder="Nome (ex: Nubank, Tesouro, Cripto)..."
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1">
              ...
              {BANK_EMOJIS.map(e => (
                <button key={e} onClick={() => setNewAreaEmoji(e)}
                  className={`text-lg px-1 rounded ${newAreaEmoji === e ? "bg-accent ring-1 ring-primary" : "hover:bg-accent/50"}`}
                >{e}</button>
              ))}
            </div>
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 w-full px-4 py-3 rounded-xl border-2 border-dashed border-border text-sm text-muted-foreground hover:bg-accent/30 hover:text-foreground transition-colors"
        >
          <Plus className="w-4 h-4" />
          Nova Área de Investimento
        </button>
      )}
    </div>
  );
...
function InvestmentAreaCard({
  ...
  const totals = getAreaTotals(area.investments);
      ...
      <div className="flex items-center">
        <button onClick={() => setCollapsed(!collapsed)} className="flex-1 flex items-center justify-between px-5 py-4 hover:bg-accent/30 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-secondary px-2 py-0.5 rounded-full text-muted-foreground">{area.investments.length} investimentos</span>
          </div>
          <div className="flex items-center gap-3">
      ...
      {!collapsed && (
        <div className="px-5 pb-5 space-y-4 animate-fade-in">
          ...
          {area.investments.map(inv => (
            <InvestmentItem key={inv.id} investment={inv} color={area.color}
            ...
            {area.goals.map(goal => {
              ...
              return (
                <div key={goal.id} className="flex items-center gap-3 mb-2">
                  ...
                  <button onClick={() => onDeleteGoal(goal.id)} className="text-destructive hover:text-destructive/80">
          ...
          {onCreateTaskReminder && (
            <div className="border-t border-border pt-3">
              <button
                onClick={() => onCreateTaskReminder(`Verificar investimentos em ${area.name}`)}
                className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
              >
...
function AreaGrowthChart({ investments, color }: { investments: Investment[]; color: string }) {
  // Combine all investments growth data for 12 months
  const months = 12;
...
function AddInvestmentForm({ onAdd, onCancel }: {
  ...
  const handleAdd = () => {
    ...
    onAdd({
      ...
      previouslyInvested: Number(prev) || 0,
      monthlyContribution: Number(monthly) || 0,
      rateOfReturn: Number(rate) || 0,
      rateType,
